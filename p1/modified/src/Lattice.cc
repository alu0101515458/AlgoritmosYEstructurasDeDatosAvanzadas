...
/**
 * MODIFICACIÓN.
 * @brief Método que devuelve cuántas células están en el estado 1.
 * 
 * @return int 
 */
int Lattice::countAliveCells() const {
  int count = 0;
  for (long unsigned i = 1; i < cells_.size()-1; ++i) {
    if (cells_[i]->getState() == ONE) ++count;
  }
  return count;
}
...
void Lattice::nextGeneration() {
  std::cout << "Celulas vivas: " << this->countAliveCells() << std::endl;
  // Calcula el estado siguiente de cada célula del retículo.
  for (long unsigned i = 1; i < cells_.size()-1; ++i) {
    // MODIFICACIÓN: Se llama al método rule30 de la clase Cell.
