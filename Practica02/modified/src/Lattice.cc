...
void Lattice::setBorder() {
  if (getReflective()) {
    ...
    ++n_;
    for (int i = 0; i < m_; ++i) {
      temp[i]->setPosition({n_, column++});
    }
    // Añadimos las celdas temporales al final del vector de celdas
    lattice_.push_back(temp);
    column = 2;
    for (int i = 0; i < m_; ++i) {
      temp[i]->setPosition({n_+1, column++});
    }
    lattice_.push_back(temp);
    ++n_;
    ++n_;
    ...
    for (int i = 2; i < n_-2; ++i) {
      ...
      lattice_[i].insert(lattice_[i].begin(), new Cell({i,1}, DEAD));
      lattice_[i].insert(lattice_[i].begin(), new Cell({i,0}, DEAD));
      // Añadimos una nueva celda al final de la fila
      lattice_[i].push_back(new Cell({i,m_+2}, DEAD));
      lattice_[i].push_back(new Cell({i,m_+3}, DEAD));
    }
    ++m_;
    ++m_;
    ++m_;
    ++m_;
    // asignamos posiciones centrales
    for (int i = 2; i < n_-2; ++i) {
      for (int j = 2; j < m_-2; ++j) {
    ...
    for (int i = 2; i < n_-2; ++i) {
      lattice_[0][i] = lattice_[2][i];
      lattice_[1][i] = lattice_[2][i];
      lattice_[n_-1][i] = lattice_[n_-3][i];
      lattice_[n_-2][i] = lattice_[n_-3][i];
    }
    // Bucle para asignar los bordes izquierdos y derechos
    for (int i = 2; i < m_-2; ++i) {
      lattice_[i][0] = lattice_[i][2];
      lattice_[i][1] = lattice_[i][2];
      lattice_[i][m_-1] = lattice_[i][m_-3];
      lattice_[i][m_-2] = lattice_[i][m_-3];
    }
...
std::ostream& operator<<(std::ostream& os, const Lattice& lattice) {
  if (lattice.getReflective()) {
    for (int i = 2; i < lattice.getRows()-2; ++i) {
    for (int j = 2; j < lattice.getColumns()-2; ++j) {
      os << lattice[{i,j}];
    }
    os << std::endl;
    }
